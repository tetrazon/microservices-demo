...
public interface ScoreRepository extends CrudRepository<ScoreCard, Long> {
    /**
     * Gets the total score for a given user: sum of the scores
     * of all their ScoreCards
     ...
     */
    @Query("SELECT NEW " +
            "microservices.book.gamification.game.domain." +
            "LeaderBoardRow(s.userId, SUM(s.score)) FROM ScoreCard s " +
